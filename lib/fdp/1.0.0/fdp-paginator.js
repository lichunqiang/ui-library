...
define(function(require, exports, module) {
	...
	var paginator = FDP.Paginator = new Class('Paginator', {
		...
		pageTpl : '<span class="mi-paging-info fn-ml15">每页</span>' +
				  ...
				  '</select>' +
				  '<span class="mi-paging-info mi-paging-which"><input type="text" id="J_number" name="number" value="$-{current}"></span>' +
		...
		paginate : function(e) {
			...
			if (this.getOperationType(e.currentTarget.className) === 'goto') {
				...
				number.save();
				if (util.isEmpty(sv)) size.set({size: this.pageDefault.size});
				//size.save(); ?
		...
		render : function(data) {
			...
			if (!util.isEmpty(data)) {
				...
				if (sel.length) 
					this.sizeSelect = new Select({ width : 58, size : 3, zIndex : 1 }).apply(sel[0]);
