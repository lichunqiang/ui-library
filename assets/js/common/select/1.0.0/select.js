define("assets/js/common/select/1.0.0/select", [ "./selectSkin.css" ], function(require, exports, module) {
    var selectSkin = require("./selectSkin.css");
    function Select(conf) {
        ...
        this.optionHeight = this.config.optionHeight || null;
        ...
        this.toggle = function(a, b) {
            ...
            root.currentItem.className = root.currentItem.className.indexOf("ui-select-item-spliter") !== -1 ? "ui-select-item-spliter ui-select-item-current" : "ui-select-item-current";
            if (root.tmpCurrentItem && root.currentItem !== root.tmpCurrentItem) {
                root.tmpCurrentItem.className = root.tmpCurrentItem.className.indexOf("ui-select-item-spliter") !== -1 ? "ui-select-item-spliter" : "";
            }
            root.tmpCurrentItem = null;
        };
        this.setCurrent = function(o) {
            ...
            for (var index in this.items) {
                if (root.currentItem == this.items[index]) {
                    ...
                    root.onSelect && root.onSelect.call(root);
                }
            }
        };
        ...
        this.add = function(item_conf, updateToSelect) {
            ...
            item.onmouseover = function() {
                root.clearTimer();
                root.currentItem.className = root.currentItem.className.indexOf("ui-select-item-spliter") !== -1 ? "ui-select-item-spliter" : "";
                if (root.tmpCurrentItem) {
                    root.tmpCurrentItem.className = root.tmpCurrentItem.className.indexOf("ui-select-item-spliter") !== -1 ? "ui-select-item-spliter" : "";
                }
                root.tmpCurrentItem = this;
        ...
        this.prepare = function(select) {
            ...
            if (opts.length > this.size) {
                this.isOverFlow = true;
                this.list.style.height = (this.optionHeight ? this.optionHeight : (this.itemHeight + 1) * this.size) + "px";
        ...
        this.iframe = this.getDOM("iframe", {
            src: "javascript:''",
            frameBorder: "0",
            scrolling: "no",
            className: "ui-select-iframe"
        });
        ...
        if (this.config.hideList && this.config.hideList === true) {} else {
